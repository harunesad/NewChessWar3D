...
{
    public bool hasMoved; // Filin daha önce hareket edip etmediðini gösteren bir özellik

    public Bishop(Player player, Vector2Int position) : base(PieceType.Bishop, player, position)
    {
        hasMoved = false; // Baþlangýçta filler henüz hareket etmemiþtir
    }
    ...
    // Fil hareket kurallarý
    public override bool Algorithm(Vector2Int destination)
    {
        ChessBoard.board[z, x] = null;

        // Fil, çapraz yönde hareket eder
        if (Mathf.Abs(destination.x - position.x) == Mathf.Abs(destination.y - position.y))
        {
